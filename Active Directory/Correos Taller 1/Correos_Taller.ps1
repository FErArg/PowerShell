...
foreach ($ou in $OUs){
    ...
    foreach ($user in $users)
    {
        ...
        If (($daystoexpire -ge "0") -and ($daystoexpire -lt $expireindays))
        {
            ...
            $email.Cc = "soporte.team@ferarg.com" 
            $email.Subject = "$Name - Aviso de Contraseña próxima a Caducar"
            $email.Body = "Buenas $Name,
            
    Su clave de usuario/correo electrónico expira en $daystoexpire días.
...
$email.Cc = "soporte.team@ferarg.com"
$email.Subject = "$FechaLog - Detalle Avisos de Claves Usuario Caducan proximamente"
